...
const AVAILABLE_SUBJECTS_FILE = path.join(__dirname, 'all_clips_list.txt'); // Generated once per day or on demand

// Helper to load available subjects from your library (file scan or pre-made list)
function getAvailableSubjects() {
  // Prefer reading from a cached file (generated by a cron job or upload)
  try {
    const lines = fs.readFileSync(AVAILABLE_SUBJECTS_FILE, 'utf-8')
      .split('\n')
      .map(l => l.trim())
      .filter(Boolean);
    const subjectSet = new Set();
    for (let line of lines) {
      // Extract main subject from filename (customize as needed!)
      let subj = line.replace(/\.(mp4|mov|webm|mkv)$/i, '')
        .replace(/_/g, ' ')
        .replace(/[\d\-]+/g, '')
        .replace(/[^\w\s]/g, '')
        .trim()
        .toLowerCase();
      if (subj && subj.length > 2) subjectSet.add(subj);
    }
    return Array.from(subjectSet);
  } catch (err) {
    console.warn('[getAvailableSubjects] Failed to load subject list:', err.message);
    return ['cat', 'dog', 'paris', 'sandwich', 'eagle', 'lion', 'pizza']; // Fallback
  }
}
...
app.post('/api/generate-script', async (req, res) => {
  ...
  console.log('[DEBUG] Library subjects loaded:', availableSubjects.length);

  // Find best matching subject for the user's idea (optional: fuzzy search)
  const userIdeaClean = idea.toLowerCase();
  let mainSubject = availableSubjects.find(subj => userIdeaClean.includes(subj));
  if (!mainSubject) {
    // Fuzzy fallback if no direct match
    const bestMatch = stringSimilarity.findBestMatch(userIdeaClean, availableSubjects);
    mainSubject = bestMatch.bestMatch.rating > 0.2 ? bestMatch.bestMatch.target : availableSubjects[0];
    console.log('[DEBUG] Fuzzy match for user idea:', mainSubject, `(score: ${bestMatch.bestMatch.rating})`);
  }

  console.log('[DEBUG] Generating script for subject:', mainSubject);

  try {
    const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });

    // This prompt **strictly limits** the script to your library subjects
    const scriptPrompt = `
You are an expert at generating YouTube Shorts scripts that perfectly match the available video clips in our library. ONLY use the following subjects as the theme or in the facts:
${availableSubjects.map((s, i) => `${i + 1}. ${s}`).join('\n')}
If the requested topic is not available, **pick the closest subject** from the list above and generate the script for that.

For the chosen subject, generate a YouTube Shorts script with 7–12 punchy lines (one per line). Each line should be crisp, voice-friendly, and natural. DO NOT use emojis, lists, numbers, or repeat words/phrases. NO generic filler. Each line must be unique and relevant to the chosen subject from the above list.
...
`.trim();
    ...
    console.log('[DEBUG] Sending OpenAI prompt to generate script:', mainSubject);
    ...
    const { viralTitle, viralDesc, viralTags } = await generateViralMetadata({
      script, topic: mainSubject, oldTitle: '', oldDesc: ''
    });
